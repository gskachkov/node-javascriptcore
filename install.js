...
/*
 * This simply fetches the right version of jsc for the current platform.
 */
...
var customDownloadUrl = process.env.JSC_DOWNLOAD_URL || false
//macosx/jsc.zip
var url = 'https://raw.githubusercontent.com/gskachkov/webkit/master/bin/';
var cdnUrl = process.env.JSC_CDNURL || url
var jscVersion = process.env.JSC_VERSION || helper.version
...
// bin for this package not the actual jsc bin.  Also help out people who
...
// If the user manually installed JSC, we want
// to use the existing version.
//
// Do not re-use a manually-installed JSC with
// a different version.
//
// Do not re-use an npm-installed JSC, because
// that can lead to weird circular dependencies between
// local versions and global versions.
// https://github.com/gskachkov/node-javascriptcore/issues
var whichDeferred = kew.defer()
which('jsc', whichDeferred.makeNodeResolver())
whichDeferred.promise
  .then(function (result) {
    ...
    if (jscPath.indexOf(path.join('npm', 'jsc')) !== -1) {
    ...
    var contents = fs.readFileSync(jscPath, 'utf8')
    if (/NPM_INSTALL_MARKER/.test(contents)) {
      ...
      cp.execFile(jscPath, ['--version'], checkVersionDeferred.makeNodeResolver())
      return checkVersionDeferred.promise
    }
  })
  .then(function (stdout) {
    var version = stdout.trim()
    if (jscVersion == version) {
      writeLocationFile(jscPath);
      console.log('JSC is already installed at', jscPath + '.')
      exit(0)

    } else {
      console.log('JSC detected, but wrong version', stdout.trim(), '@', jscPath + '.')
  ...
  .then(function (extractedPath) {
    console.log('copy', extractedPath, pkgPath);
    return copyIntoPlace(extractedPath, pkgPath)
  })
  .then(function () {
    console.log('pkg file', pkgPath);
    var location = process.platform === 'win32' ?
        path.join(pkgPath, 'jsc.exe') :
        path.join(pkgPath, 'jsc')
    ...
    console.log('Done. jsc binary available at', location)
    exit(0)
  })
  .fail(function (err) {
    console.error('jsc installation failed', err, err.stack)
    exit(1)
  })
...
function findSuitableTempDirectory(npmConf) {
  ...
  for (var i = 0; i < candidateTmpDirs.length; i++) {
    var candidatePath = path.join(candidateTmpDirs[i], 'jsc')
  ...
  console.error('Can not find a writable tmp directory, please report issue ' +
      'on https://github.com/gskachkov/node-javascriptcore/issues with as much ' +
...
function requestBinary(requestOptions, filePath) {
  ...
  requestProgress(request(requestOptions, function (error, response, body) {
    ...
    if (!error && response.statusCode === 200) {
      ...
      console.error('Error making request, SELF_SIGNED_CERT_IN_CHAIN. Please read https://github.com/gskachkov/node-javascriptcore/issues')
      exit(1)
    } else if (error) {
      console.error('Error making request.\n' + error.stack + '\n\n' +
          'Please report this full log at https://github.com/gskachkov/node-javascriptcore/issues')
      exit(1)
    } else {
      console.error('Something unexpected happened, please report this full ' +
          'log at https://github.com/gskachkov/node-javascriptcore/issues')
      exit(1)
    }
...
function copyIntoPlace(extractedPath, targetPath) {
  ...
  return kew.nfcall(rimraf, targetPath).then(function () {
    ...
    if (fs.statSync(extractedPath).isDirectory()) {
      console.log('Copying extracted folder', extractedPath, '->', targetPath)
  ...
  .then(function () {
    ...
    return kew.nfcall(rimraf, extractedPath).fail(function (e) {
      ...
      console.warn('Unable to remove temporary files at "' + extractedPath +
          '", see https://github.com/gskachkov/node-javascriptcore/issues for details.')
    })
  })
}
